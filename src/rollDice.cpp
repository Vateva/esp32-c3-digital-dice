...
void rollDice() {
  ...
  int totalDice = coinCount + d4Count + d6Count + d8Count + d10Count + d12Count + d20Count;
  ...
  for (int globalFrame = 0; globalFrame < totalFrames; globalFrame++) {
    ...
    for (int die = 0; die < totalDice; die++) {
      ...
      const unsigned char* const* currentDieBitmapArray = getDiceBitmapArray(diceArray[die].type, use64x64);
      ...
      if (globalFrame >= dieStartFrame) {
        ...
        if (currentDieFrame < framesPerDie) {
          ...
          display.drawBitmap(xPositions[die], yPositions[die], 
                             currentDieBitmapArray[bitmapIndex], bitmapSize, bitmapSize, WHITE);
        } else {
          //die has finished animating - show final result
          int finalBitmapIndex = 11 + ((diceArray[die].result - 1) * 7) + 6; //last frame of result
          display.drawBitmap(xPositions[die], yPositions[die], 
                             currentDieBitmapArray[finalBitmapIndex], bitmapSize, bitmapSize, WHITE);
        }
      }
    }
