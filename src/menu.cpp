...
void drawMenu() {
  ...
  case MENU_MAIN:
    ...
    for (int i = 0; i < MAIN_MENU_ITEMS; i++) {
      display.setCursor(0,
                        12 + (i * 13)); // positions each item 12 pixels apart
  ...
  case MENU_HISTORY:
    ...
    if (historyCount == 0) {
      ...
      for (int i = 0; i < entriesToShow; i++) {
        display.setCursor(0,
                          16 + (i * 7)); // positions each entry 7 pixels apart
...
void loadConfiguration() {
  ...
  if (brightness > 255)
    brightness = 255;
  if (clearDisplayTime < 1000)
    clearDisplayTime = 3500;
  if (clearDisplayTime > 10000)
    clearDisplayTime = 3500;
  if (stagger < 0)
    stagger = 2;
  if (stagger > 20)
